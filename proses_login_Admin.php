...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        header('Location: laporan.php'); // Ganti ke halaman dashboard kamu
        exit();
    } else {
        // Login gagal
        header('Location: login Admin.php?error=Username atau Password salah!');
        exit();
    }
} else {
    // Jika username atau password tidak dikirimkan
    header('Location: login Admin.php?error=Username atau Password tidak valid!');
    exit();
}
?>
